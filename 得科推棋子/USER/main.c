...
int off_stage()
{
	ltxback=0;
	//台下程序
	if( (AI(2) > 2000  && (AI(8)>1100 || AI(14)>1100)  &&  AI(12)<600 &&  AI(6) <550 && AI (10) <550))
	{
		up_stage_delay+=1;
		if(up_stage_delay>3)
		{
...
int turn_to_attack()
{
	...
	*/
	//推棋子：增加前2 检测距离，禁用尾2
		while(1)
		{
//			if(analog_scan_close()==2 || analog_scan_close()==3) return 1;
...
//			}
			...
			if(AI(2)>attack_dis-100 || AI(3)>attack_dis-100) return 1;
			else if(AI(1)>= attack_dis || AI(11)>=attack_dis || AI(10)>= attack_dis || AI(9)>=attack_dis)
			{
				while(AI(2)<attack_dis && AI(3)<attack_dis)
				{
					GoodMoto(-200,200);
				}
				continue;
